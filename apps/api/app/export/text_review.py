...
from app.export.content import content_digest, is_released, text_fields
...
class ExportTextReviewRequest(ContentReviewRequest):
    pass


class ExportTextReviewResponse(ContentReviewSnapshot):
    kind: ExportTextKind
...
def require_record(
    ...
    *,
    lock: bool = False,
) -> CustomsDeclaration | TaxRefundCase:
    context.require(Permission.PROFIT_READ)
    context.require(Permission.EXPORT_READ)
    if kind == "customs":
        model: type[CustomsDeclaration] | type[TaxRefundCase] = CustomsDeclaration
    elif kind == "refund":
        model = TaxRefundCase
    else:
        raise ApiProblem(404, "EXPORT_TEXT_NOT_FOUND", "Not found", "Unsupported EXPORT record.")
    ...
    if not isinstance(row, (CustomsDeclaration, TaxRefundCase)):
        raise ApiProblem(404, "EXPORT_TEXT_NOT_FOUND", "Not found", "EXPORT record not found.")
    return row


def snapshot(row: CustomsDeclaration | TaxRefundCase) -> ExportTextReviewResponse:
    return ExportTextReviewResponse(
        record_id=row.id,
        kind="customs" if isinstance(row, CustomsDeclaration) else "refund",
        version=row.version,
        content_digest=content_digest(row),
        text="报关备注及退回原因" if isinstance(row, CustomsDeclaration) else "退税备注及退回原因",
...
class ExportTextReviewService:
    def __init__(self, factory: sessionmaker[Session]) -> None:
        self.factory = factory

    def inspect(
        self, context: RequestContext, kind: ExportTextKind, record_id: UUID
