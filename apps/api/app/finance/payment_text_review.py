...
class PaymentTextReviewResponse(ContentReviewSnapshot):
    pass
...
def require_record(
    ...
    *,
    ...
    if not isinstance(row, Payment):
        raise ApiProblem(404, "PAYMENT_TEXT_NOT_FOUND", "Not found", "Payment record not found.")
    return row


def snapshot(row: Payment) -> PaymentTextReviewResponse:
    return PaymentTextReviewResponse(
        record_id=row.id,
        version=row.version,
...
class PaymentTextReviewService:
    def __init__(self, factory: sessionmaker[Session]) -> None:
        self.factory = factory

    def inspect(self, context: RequestContext, record_id: UUID) -> PaymentTextReviewResponse:
