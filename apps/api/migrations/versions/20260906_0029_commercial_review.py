"""Preserve historical commercial text and default its disclosure to confidential."""
...
def upgrade() -> None:
    for table in ("products", "inquiries", "quotation_versions", "sales_orders", "sales_contracts"):
...
def downgrade() -> None:
    if op.get_bind().scalar(
        sa.text(
            "SELECT EXISTS (SELECT 1 FROM products) OR EXISTS (SELECT 1 FROM inquiries) "
            "OR EXISTS (SELECT 1 FROM quotation_versions) OR EXISTS (SELECT 1 FROM sales_orders) "
            "OR EXISTS (SELECT 1 FROM sales_contracts)"
        )
    ):
        raise RuntimeError(
            "Commercial evidence exists; preserve confidentiality with a forward migration."
        )
    for table in ("products", "inquiries", "quotation_versions", "sales_orders", "sales_contracts"):
