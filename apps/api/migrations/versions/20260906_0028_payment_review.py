"""Preserve historical Payment text and default its disclosure to confidential."""
...
def downgrade() -> None:
    if op.get_bind().scalar(sa.text("SELECT EXISTS (SELECT 1 FROM payments)")):
        raise RuntimeError(
            "Payment evidence exists; preserve confidentiality with a forward migration."
        )
    for table in ("payments",):
