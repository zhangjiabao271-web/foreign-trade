"""Preserve historical CRM text and default its disclosure to confidential."""
...
def downgrade() -> None:
    if op.get_bind().scalar(
        sa.text("SELECT EXISTS (SELECT 1 FROM leads) OR EXISTS (SELECT 1 FROM opportunities)")
    ):
        raise RuntimeError(
            "CRM evidence exists; preserve confidentiality with a forward migration."
        )
    for table in ("leads", "opportunities"):
