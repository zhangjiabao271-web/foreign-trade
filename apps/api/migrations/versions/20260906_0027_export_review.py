"""Preserve historical Export text and default its disclosure to confidential."""
...
def upgrade() -> None:
    for table in ("customs_declarations", "tax_refund_cases"):
...
def downgrade() -> None:
    if op.get_bind().scalar(
        sa.text(
            "SELECT EXISTS (SELECT 1 FROM customs_declarations) "
            "OR EXISTS (SELECT 1 FROM tax_refund_cases)"
        )
    ):
        raise RuntimeError(
            "Export evidence exists; preserve confidentiality with a forward migration."
        )
    for table in ("customs_declarations", "tax_refund_cases"):
