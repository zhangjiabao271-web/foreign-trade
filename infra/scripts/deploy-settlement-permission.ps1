...
$expectedImage = 'sha256:1da4ade3822607f541a5fea3e716b5a35151cb2eed7b68902f699b69343920d5'
...
try {
    ...
    $image = docker image inspect 'trade-fresh-acceptance-api:settlement-permission-20260909' --format '{{.Id}}'
    if ($LASTEXITCODE -ne 0 -or $image -ne $expectedImage) { throw 'Unexpected candidate image' }
    $api = Read-Service 'api'
    if ($api.Image -ne 'sha256:7f55625e034bab39ac16e2862c7de6319416828015b667c9a013c26b949b8c53') {
        throw 'Unexpected source API image; inspect before retrying'
    }
    $activeRuns = docker exec "${project}-postgres-1" psql -U rehearsal -d trade_fresh_acceptance -qAtc "SELECT count(*) FROM ai_runs WHERE status IN ('PENDING','RUNNING')"
    if ($LASTEXITCODE -ne 0 -or "$activeRuns" -ne '0') { throw 'AI runs must be idle before deployment' }
    $unchanged = @{}
