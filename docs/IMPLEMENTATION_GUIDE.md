...
ADR-027（用户已确认）补充垫资估算：取“订单报价成本＋净额外费用－本订单净核销收款”与零的较大值，统一订单币种，冲销扣回、不重复计入报价已含费用。仅管理员／经理／财务按原读取权限可见；明确标注为估算，不代表实际现金缺口或按收付款时点计算的资金峰值，不改变资金事实。
...
ADR-020（用户已确认）收紧成本与利润：仅 ADMIN、MANAGER、FINANCE 可见；SALES、OPERATIONS、VIEWER 不得通过接口、命令返回、时间线或 AI 读取内部成本和利润。销售草稿通过后端继承受保护成本快照，成本维护与采购定价交由已有授权的经理／管理员处理；运营保留不含采购金额的数量、日期和履约操作。不得用前端隐藏代替服务端权限，也不得把隐藏值当作零或回写历史事实。详见 ADR 的字段、工作流及验收边界。
...
ADR-014 补充：询盘/报价通过 CRM 事务端口推进商机；洽谈与丢单使用有版本、原因及幂等键的显式命令。赢单仅来自报价接受，丢单终态不得被后续接受覆盖；V1 不提供手工重开。
...
ADR-021 收紧发布前的 submit/approve/send/accept/reject/expire 接口：必须提交页面所见的报价版本 ID、记录版本号及幂等键。新操作版本不符时拒绝，不得静默选择后来的当前版本；相同操作重试返回原受影响版本的受保护现状。前后端和测试调用者必须同时更新，主环境部署另行验证。这是未发布 V1 的显式破坏性契约调整，不宣称兼容无版本保护的旧调用。

### 5.4 Sales Order

ADR-023 补充建单请求身份：可选幂等键与订单快照、编号和证据同事务提交；同键原样重试返回原订单的受保护现状。每个接受报价仍只允许一张订单，但新的重复请求必须与原定金比例、到期日一致，否则明确冲突，不得把不同条件静默当作成功。这是未发布 V1 的显式冲突语义调整，不提供修改已存订单条件的旁路。

ADR-022 收紧发布前的订单 confirm 接口：必须提交所见订单版本号及幂等键；过时的新操作拒绝执行，同一操作重试返回原订单受保护的现状，不重复生成采购任务。前后端和调用者同步升级，无省略版本的兼容旁路；不修改金额、定金、权限和完成规则，主环境部署另行验证。
...
ADR-015 补充销售合同：从订单复制不可变商业快照，草稿可维护备注和外部编号；经理以已验收且固定存储版本的合同文件登记人工签署事实。签署不改变订单或资金状态；仅草稿可作废，已签合同不可覆盖或直接作废。

### 5.5 Purchase Order

ADR-024 收紧发布前采购 approve/send/confirm：必须提交所见采购版本号和幂等键；按销售订单、采购单顺序加锁。相同操作重试返回原采购的受保护现状；新操作拒绝过时版本与已终结销售订单。供应商确认号和交期不同不得静默成功或覆盖原承诺；这是显式契约调整，前后端调用者同步升级，不改变采购金额、数量和收货事实。
...
ADR-013 补充：取消仅释放未收货部分，保留原订货/价格/收货事实；变更通过同事务取消剩余承诺并生成关联的新采购草稿，重新审批与确认，不直接覆盖已确认采购快照。

### 5.6 Shipment

ADR-025 收紧发布前七个出运节点：必须提交所见出运版本号及幂等键；按订单、出运单顺序加锁。原样重试返回原出运单当前事实和文件缺口，不重复节点或证据。新请求检查版本、关联订单与数量，终结订单不得继续推进；不同订舱号不得静默成功或覆盖。前后端同步升级，不提供无版本兼容旁路，不改变文件要求、独立时间戳和 Sales 所有的订单推进规则。
...
ADR-016 补充订单费用：财务登记已发生的附加费用及汇率快照，明确区分额外成本和已包含报价成本，避免重复扣减。更正追加独立冲销事实；不改写报价、订单或付款。费用调整后的报价毛利仅作预测，不称为最终实际利润。

ADR-017 补充供应商结算：人工核对应付采购本金与付款凭证，付款和核销分离；同供应商、同币种核销不得超过付款可用额或应付余额。采购取消不自动抵销债务，付款更正保留反向事实；不发起实际银行转账，不改动客户收款和订单完成规则。
...
ADR-030 落实询盘所有权：创建报价通过 Inquiries 的事务端口推进询盘，保持原报价事务和锁顺序；已关闭询盘不得被新报价重新开放，返回明确状态冲突。既有报价重试、历史记录和接口结构不变。

ADR-031 落实客户档案所有权：线索转换通过 Companies 的事务端口创建或关联公司、客户角色和联系人；端口独立检查原 lead.convert 权限，不自行提交。CRM 保留线索锁、商机、转换幂等和原有证据；公司去重、租户隔离、字段与回滚语义不变。

ADR-032 落实确认订单的任务所有权：Work 通过 Sales 的受限事实端口复核已确认订单和原 order.confirm 权限，在原事务内创建采购准备任务。保持两天到期、高优先级、幂等及证据规则，不借用 AI 审批、不单独提交。

ADR-033 落实异步作业所有权：AI 和 Documents 通过 Platform 的事务端口创建及更新作业，保留各领域锁、重试、租约和事件。请求端复核原权限；内部 worker 失败记录不依赖用户仍持有发起权限，确保撤权失败也能留证。端口校验组织及作业类型，不单独提交或重复投递。

ADR-034 统一通过 Work 记录活动证据：领域调用者保留原权限、对象归属、锁和事务；记录器只暂存原字段，不查询、提交、审核开放或递归生成证据。用户来源与后台无操作者来源分开，保留关联编号、数据库时间及默认保密规则。
...
ADR-028 统一发布前请求校验及框架 HTTP 错误为上述结构；校验错误仅返回固定来源与错误类别，不回显输入值、任意字段名或校验器正文。既有业务错误码、权限顺序和 HTTP 状态保持不变。
...
部署时以 `OIDC_SIGNING_ALGORITHM` 明确固定可信发行方的签名算法（RS256 或 ES384；兼容默认 RS256）。先核实发行方的公开配置和密钥；不得根据未验证令牌自动放宽算法。当前本地 Logto 使用 ES384／P-384，配置一致性不替代真实登录验收。

### 10.2 强制隔离

ADR-012 补充浏览器登录：官方 Logto Next.js SDK 管理服务端加密 HttpOnly 会话；同源代理附加业务 API token，并拒绝跨源写入。浏览器只保留组织选择与非授权会话标识，令牌粘贴仅可用于明确启用的隔离测试。登录后只列出当前用户的有效本地成员组织，不自动创建成员或提权。

ADR-018 补充组织管理：ADMIN 只管理当前组织名称、业务时区和成员授权。核实准确 Logto subject 后可显式建立本地有效成员，但不创建远程账号、密码或自动邀请，不修改已有全局用户。成员角色调整、停用与重新启用均由命令执行；锁定当前组织后重新核验管理员，禁止移除最后一位有效管理员，并原子记录理由、审计、activity、outbox 和幂等结果。
...
ADR-010 补充：对象存储启用版本控制；complete 校验并在 PostgreSQL 固定具体 storage version ID，下载只签名该版本，防止有效期内重复 PUT 改写已验收证据。旧记录重新校验后固定版本，迁移不删除旧文件。

ADR-020 补充（用户已确认）：附件及自由文本默认保密，存量内容也需审核。管理员、经理或财务确认不含内部成本／利润后，才能向销售、运营和普通只读角色开放，且仍需原业务读取权限。审核绑定实际内容与具体版本，内容变化须重新审核；原始内容及审核证据必须保留。该人工审核不代替恶意文件扫描。

用户进一步确认：公司名、联系人姓名、产品名称等业务识别字段按原业务权限保持可见，禁止在其中填写内部成本或利润；备注、说明、条款和历史正文仍按审核后开放处理。这是明确的字段使用规则，不代表系统已自动识别或清除其中的敏感信息。

## 13. AI 权限边界

ADR-026（用户已确认）：创建者可主动提交指定私有草稿给管理员、经理或财务做内容审核；
未提交运行仍保持私有，创建者身份不能绕过成本/正文保密。审核仅开放确认过的具体候选版本，
脱敏追加版本而不改写原稿，修改须重审；不得开放整段对话或无关附件。内容可见性审核与
发送、改价、任务执行等业务审批独立，详见 ADR-026。
...
ADR-011 冻结初版受控 Copilot：明确意图、有限只读工具、异步运行、实时复核权限；先开放人工批准的订单跟进任务创建，不开放 AI 发送邮件或改变核心商业/资金状态。未配置模型服务时明确不可用，不以测试替身冒充真实模型。

ADR-029（用户2026-09-08选择）：增加显式 DeepSeek V4 Pro 适配器，模型标识 deepseek-v4-pro，
仅使用官方接口与独立环境密钥，不自动切换模型或供应商。运行保存供应商限定的模型身份；
既有工具、权限、证据、次数限制和人工审批保持不变。真实验收仅使用合成业务数据。

## 14. 可观测性、安全与运营基线

ADR-019 明确运行观测边界：日志采用白名单 JSON，组织业务统计来自 PostgreSQL 且仅管理员读取；接口临时样本有明确进程/重置窗口，共享连接池数据只写运维日志。未知 AI 费用和无已决请求批准率不得伪装为零；监控失败不能改变业务事务结果。
